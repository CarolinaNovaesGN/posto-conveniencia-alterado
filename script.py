...
def user_score():
    ...
    user_stars = int(
        input("\nDe 0 a 5 estrelas, quão satisfeito vocé está com nosso atendimento? "))
        
    while user_stars not in range(1, 6):
        user_stars = int(input("\nPor favor, digite um número de 0 a 5! "))
...
while access_control in ['A','a']:
    ...
    if access_control in ['A','a']:
        ...
        (N) Não. quero continuar conprando''')
                    if quit in ['S','s']:
                        all_products = []
                        access_control = 'a'
                    elif quit in ['N','n']:
        ...
        Você ainda não realizou uma compra''' + color.END)
                ...
                if all_products:
                    print("ATENÇÃO! Pague suas compras antes de sair! ")
                    products.basket(all_products)
                else:
                    access_control = 'a'
                    break
